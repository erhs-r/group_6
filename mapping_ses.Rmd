...
```{r pivotlong}
#Pivot df with geodata and income vars longer, names=income vars and their values
...
fig_pop <- ggplot(data_map) +
  ...
  geom_point(aes(x=lon, y=lat), town_latlon) +
...
fig_below <- larimer_tracts %>% left_join(.,larimer_short, by = "TRACTCE") %>%
  ggplot() +
