...
poly <- pop <-  tracts_income %>%
  ggplot() +
  geom_sf() +
  geom_point(data = LC_covid_gps %>%
               ...
               geom_text_repel(data = larimer_gps %>%
                        ...
                        show.legend=NA, 
                        nudge_x = .1, nudge_y = .1) +
    ggtitle("Larimer County City Boundaries") + theme_bw()
poly
...
pop <-  tracts_income %>%
  ...
  geom_sf(aes(fill = Population_Total)) +
   geom_point(data = mean_age_deaths %>%
               filter(City!="Cedaredge"), aes(x=Lon, y=Lat)) +
               geom_text_repel(data = larimer_gps %>%
                        filter(City!="Cedaredge"), 
                        aes(x=Lon, y=Lat, 
...
pop_den <-  tracts_income %>%
  ...
  scale_fill_viridis(name = "Number of People Per Square Mile", direction = -1, option = "cividis") +
...
comp_plot <- ggarrange(pop, pop_den, inc, pov_inc, pov,
                       ncol = 23, nrow = 2, 
                       labels = c("a", "b", "c", "d", "e"))

#ggsave() then specify the width and height
```
