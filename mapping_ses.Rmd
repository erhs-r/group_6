...
#create base figure with city names as points
cases_fig <- tracts_income %>%
  ggplot() +
  geom_sf() +
  geom_point(data = cases %>%
               filter(City!="Cedaredge"), aes(x=Lon, y=Lat, size = n)) +
               geom_text_repel(data = cases %>%
                        filter(City!="Cedaredge"), 
                        aes(x=Lon, y=Lat,
                        label = City), color = "black",
                        show.legend=NA, 
                        nudge_x = .1, nudge_y = .1) +
    ggtitle("Larimer County COVID-19 Cases") + theme_void() 
cases_fig
...
pop <-  tracts_income %>%
  ...
  geom_sf(aes(fill = Population_Total)) +
   geom_point(data = cases %>%
               filter(City!="Cedaredge"), aes(x=Lon, y=Lat, size = n),
                                              show.legend = FALSE) +
  scale_fill_viridis(name = "Population", direction = -1, option = "cividis") +
  ggtitle("Population by City") + theme_void()
pop

#Population density
pop_den <-  tracts_income %>%
  ggplot() +
...
#percent total pop below poverty map
pov <- tracts_income %>%
  ggplot() +
...
comp_plot <- ggarrange(cases_fig, pop, pop_den, inc, pov,
                       heights = c(3, 3, 3, 3),
                       widths = c(3, 3, 3, 3),
                       nrow = 2,  ncol = 4, 
                       labels = c("a", "b", "c", "d", "e"))
...
* Likely will not use this code and can remove for the final draft
